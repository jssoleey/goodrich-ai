...
from .llm import get_chatbot_response, get_script_response, get_kakao_response, get_random_cancel_info
import re
import os, json
    ...
    if st.session_state.page == "input":
        ...
        with col1:
            if st.button("📝 입력 예시 불러오기", use_container_width=True):
                example_situation = "나에게 2023년 6월 태아보험을 가입해서 2년동안 잘 유지하고 있는데 박람회서 보험설계사를 만나서 내가 설계한 내용중 암담보가 너무 작고 태아때 특약을 빼고 어린이에 맞게 가입시켜준다고 해지 할려고 합니다. 태아담보는 내가 조정해서 보험료를 낮춰주고 다른 특약을 넣어서 설계해 준다고 하니 왜 이제와서 해주냐고 박람회 설계사에게 한다고 하네요... 많이 속상한데 이분이 나한테 계약을 유지하고 신뢰할수 있는 맨트를 만들어 주세요"
                st.session_state['customer_situation_input'] = example_situation
                st.experimental_rerun()
